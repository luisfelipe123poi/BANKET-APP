...
# --- CONFIGURACIÓN PARA TRADEROS ---

def init_trader_db():
    conn = get_db_connection()
    # Tabla de estudiantes y métricas
    conn.execute('''
        CREATE TABLE IF NOT EXISTS students (
            ...
            avatar TEXT,
            joined TEXT,
            stats_trades INTEGER DEFAULT 0,
            stats_wins INTEGER DEFAULT 0,
            stats_losses INTEGER DEFAULT 0,
            stats_profit REAL DEFAULT 0.0,
...
# Ejecutar la creación de tablas al cargar el script
init_trader_db()
...
def trader_login():
    ...
    # Mentor Master (puedes cambiar esta clave)
    if password == "admin123": 
        return jsonify({"ok": True, "role": "mentor"})
    ...
    if student:
        return jsonify({
            ...
            "name": student['name'],
            "metrics": {
                "trades": student['stats_trades'],
                "wins": student['stats_wins'],
                "losses": student['stats_losses'],
                "profit": student['stats_profit'],
                "score": student['stats_score']
            }
        })
...
def register_student():
    ...
    try:
        conn = get_db_connection()
        conn.execute('''
            INSERT INTO students (email, name, country, password, role, joined)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (data['email'], data['name'], data['country'], data['password'], 'student', data['joined']))
        conn.commit()
        conn.close()
        return jsonify({"ok": True})
    except Exception as e:
...
def update_metrics():
    ...
    conn.execute('''
        UPDATE students SET 
        stats_trades=?, stats_wins=?, stats_losses=?, stats_profit=?, stats_score=?
        WHERE email=?
    ''', (s['trades'], s['wins'], s['losses'], s['profit'], s['score'], email))
    conn.commit()
    conn.close()
    return jsonify({"ok": True})
...
def get_user_data():
    ...
    # Obtener lista completa de alumnos para el mentor
    rows = conn.execute('SELECT * FROM students').fetchall()
    student_row = conn.execute('SELECT * FROM students WHERE email=?', (email,)).fetchone()
    conn.close()

    students_list = [{
        "email": r['email'], "name": r['name'], "country": r['country'],
        "stats": {"trades": r['stats_trades'], "wins": r['stats_wins'], 
                  "losses": r['stats_losses'], "profit": r['stats_profit'], "score": r['stats_score']}
    } for r in rows]

    if student_row:
        return jsonify({
            "role": "student",
            "metrics": {"trades": student_row['stats_trades'], "wins": student_row['stats_wins'], 
                        "losses": student_row['stats_losses'], "profit": student_row['stats_profit'], 
                        "score": student_row['stats_score']},
            "students": students_list
        })
    return jsonify({"role": "mentor", "students": students_list})
